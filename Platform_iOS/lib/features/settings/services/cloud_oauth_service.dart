...
// Provider credentials are injected at build time via --dart-define.
...
// Custom URL scheme used for the Dropbox redirect on iOS. It must also be
// listed under CFBundleURLTypes in ios/Runner/Info.plist and registered as a
// redirect URI ("<scheme>://oauth2redirect") in the Dropbox app console.
const cloudOAuthRedirectScheme = String.fromEnvironment(
  'CLOUD_OAUTH_REDIRECT_SCHEME',
...
/// Reversed-client-ID URL scheme Google requires for iOS OAuth clients, e.g.
/// `123-abc.apps.googleusercontent.com` -> `com.googleusercontent.apps.123-abc`.
/// This scheme must be registered under CFBundleURLTypes in Info.plist.
String _googleReversedClientId(String clientId) {
  final match = RegExp(r'^(.+)\.apps\.googleusercontent\.com$')
      .firstMatch(clientId.trim());
  if (match == null) return '';
  return 'com.googleusercontent.apps.${match.group(1)}';
}
...
/// iOS account sign-in for Google Drive / Dropbox. Uses
/// ASWebAuthenticationSession (via flutter_web_auth_2) with a custom URL-scheme
/// redirect — the desktop loopback-redirect server is not used on iOS. Token
/// exchange, refresh, storage, and account lookups mirror the macOS service.
class CloudOAuthService {
  ...
  Future<CloudAccountConnectResult> connect(
    ...
    if (config == null) {
      return CloudAccountConnectResult(
        connected: false,
        unsupported: true,
        message: '${provider.label} direct account sign-in is not available in '
            'this app yet. Use the local synced folder for now.',
      );
    }
    ...
    try {
      ...
      final authUrl = config.authorizationUri(
      ...
      final callback = _parseCallback(callbackUrl, expectedState: state);
      ...
      // flutter_web_auth_2 throws when the user cancels the sheet; treat that
      // as a benign cancellation rather than a failure.
      if (_looksLikeUserCancellation(error)) {
        return CloudAccountConnectResult(
          connected: false,
          cancelled: true,
          message: '${provider.label} sign-in was cancelled.',
        );
      }
      return CloudAccountConnectResult(
        connected: false,
  ...
  bool _looksLikeUserCancellation(Object error) {
    final text = error.toString().toLowerCase();
    return text.contains('cancel') || text.contains('canceled');
  }

  _OAuthCallback _parseCallback(
    String callbackUrl, {
    required String expectedState,
  }) {
    final uri = Uri.tryParse(callbackUrl);
    if (uri == null) {
      return const _OAuthCallback(error: 'invalid_callback');
    }
    final params = {
      ...uri.queryParameters,
      // Some providers return params in the fragment instead of the query.
      if (uri.fragment.isNotEmpty)
        ...Uri.splitQueryString(uri.fragment),
    };
    final state = params['state'];
    if (state != expectedState) {
      return const _OAuthCallback(error: 'security_state_mismatch');
    }
    return _OAuthCallback(code: params['code'], error: params['error']);
  }
  ...
  Future<Map<String, dynamic>> _postTokenRequest({
    ...
    final payload = <String, String>{...parameters};
  ...
  factory _CloudOAuthTokenException.fromTokenResponse({
    ...
    if (config.clientIdDefineName == 'GOOGLE_DRIVE_CLIENT_ID' &&
      ...
      return const _CloudOAuthTokenException(
        'Google Drive rejected the iOS OAuth client. Verify the '
        'GOOGLE_DRIVE_CLIENT_ID is an iOS-type OAuth client and that its '
        'reversed-client-ID URL scheme is registered in Info.plist.',
...
class _OAuthProviderConfig {
  ...
  String? get credentialIssue {
    if (clientIdDefineName == 'GOOGLE_DRIVE_CLIENT_ID') {
      if (!_looksLikeGoogleOAuthClientId(clientId)) {
        return 'Google Drive account sign-in is ready in code, but '
            'GOOGLE_DRIVE_CLIENT_ID is not a valid OAuth iOS Client ID.';
      }
      if (callbackScheme.isEmpty) {
        return 'Google Drive sign-in could not derive the reversed-client-ID '
            'redirect scheme from GOOGLE_DRIVE_CLIENT_ID.';
      }
    }
    return null;
  }
  ...
  factory _OAuthProviderConfig.googleDrive({required String clientId}) {
    ...
    final reversed = _googleReversedClientId(normalizedClientId);
    return _OAuthProviderConfig(
      ...
      redirectUri: reversed.isEmpty ? '' : '$reversed:/oauth2redirect',
  ...
  factory _OAuthProviderConfig.dropbox({required String clientId}) {
    ...
    return _OAuthProviderConfig(
      ...
      redirectUri: '$cloudOAuthRedirectScheme://oauth2redirect',
      callbackScheme: cloudOAuthRedirectScheme,
      authorizationUri: ({
        required String redirectUri,
        required String state,
        required String challenge,
