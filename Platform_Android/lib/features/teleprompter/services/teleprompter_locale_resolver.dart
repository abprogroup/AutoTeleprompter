...
/// Ported from Windows. `visibleTranscriptPlausiblyMatchesLocale` and
/// `shouldBlockLocaleSyncDuringAssistPin` were already present on Android as
/// static methods on `TeleprompterNotifier` (byte-identical logic, just not
/// extracted into their own file) - Android's mixed Hebrew/English
/// locale-switching feature already worked before this port. This file adds
/// the newer resolver methods Android didn't have yet
/// (`explicitLocaleForWord`, `resolveSectionLocalesForWords`,
/// `resolveInitialSttLocale`), available for future use.
///
/// Deliberately NOT done in this pass: swapping
/// `TeleprompterNotifier._precomputeSectionLocales`'s own (already
/// shipped, working) section-locale algorithm for
/// `resolveSectionLocalesForWords` below. The two differ narrowly (this one
/// excludes numbers/punctuation from directly driving locale classification
/// and inherits from neighbors instead; the existing Android algorithm
/// classifies every non-newline word by `isRtl` and smooths runs <3 words).
/// Windows' version is likely more correct, but swapping the algorithm
/// backing a live, working, safety-adjacent feature without dedicated test
/// coverage on this exact codebase is exactly the kind of change that
/// deserves its own careful pass - see `android_parity_gaps.md` #22.
