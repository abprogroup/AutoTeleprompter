...
## Windows v4.1.12 Final Seal (2026-04-29)

Windows v4 is complete and sealed at version `4.1.12+12`. The final Windows
backup is stored at `backups/final_windows_v4/20260429_103357`, and the final
release notes live at `releases/Windows/v4.1.12 (FINAL)/README.md`.

Do not change the sealed Windows v4 behavior while porting to other platforms
unless the user explicitly reopens Windows v4. Key sealed contracts:

- STT stop/start resumes from the current script point; Restart is the only
  reset-to-start command.
- Bookmarks sync between editor and presenter, support multiple anchors, can be
  added/removed in either mode, and remain usable while STT is active.
- Editor and presenter font controls share one metadata value; presenter visual
  scaling is display-only.
- Editor/presenter spacing controls use the same ranges and persisted metadata.
- Present mode preserves symbols, quotes, punctuation, and intentional blank
  lines.
- RTF/DOCX export converts app markup into document styling; raw tags must not
  spill into saved files.
- External mic selection is available from presenter settings via WebView2 audio
  input enumeration with System Default fallback.
- Debug mode uses the compact sound bar and collapsible output window; volume
  bar spam must not return to logs.

V5 preparation: split `script_editor_screen.dart` and
`teleprompter_screen.dart` into smaller behavior-owned files before adding large
new Windows features. The split must preserve the sealed contracts above and
must not be mixed with feature changes.

## Windows v4.1.12 Final Verified State (Updated 2026-04-29)

The V5-prep split described above is now complete for Windows. Every Dart file
under `Platform_Windows/lib` is below 800 lines, and the split was performed as
a behavior-preserving extraction using Dart `part` files.

Latest verified Windows v4 commit: `160d137`.
Verified workflow run: `Build Windows EXE (v4.1.12)` / `25110648732`.

Additional final STT contract:

- Default recognition may recover locally up to 5 missed words.
- `Allow visible text skip` enables longer jumps only to rendered visible text.
- Visible skipping is fallback-only after nearby 3+ word phrase priority fails,
  preventing later similar text from stealing the current phrase.

This platform is now ready for V5 development only after preserving these final
contracts. Cross-platform migration should continue next in `Platform_iOS/`.
