...
/// Ported verbatim from Windows - pure file management, no platform
/// dependency. The 'wav' format branch is unreachable on Android today
/// (Android has no separate WAV-only recorder - `wav_audio_recorder_service.dart`
/// on Windows is native `winmm.dll` FFI and doesn't apply here; Android's
/// camera recording already captures audio+video together), kept only so
/// this service's shape matches Windows' exactly if that ever changes.
