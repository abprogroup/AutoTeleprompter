...
- [ ] **Windows External Microphone Selection**: Add explicit support for an outer connected microphone instead of relying only on the Windows default input device. Current Windows browser-STT path uses the OS/default WebView microphone selected by Windows; required hardening is an in-app/default-device check, visible external mic status, and eventually a device picker that can choose a USB/Bluetooth/interface microphone before starting STT. (REQUESTED 2026-04-28)
