...
class _HistoryMenu extends StatelessWidget {
  ...
  Widget build(BuildContext context) => PopupMenuButton<int>(
        icon: Icon(Icons.history,
            size: 20, color: history.isEmpty ? Colors.white10 : Colors.white70),
        ...
        itemBuilder: (_) => history
            ...
            .map((e) => PopupMenuItem(
                value: e.key,
                child: Text(
                    '${e.value.description} (${e.value.timestamp.hour}:${e.value.timestamp.minute.toString().padLeft(2, "0")})',
                    style: TextStyle(
                        color: e.key == historyIndex
                            ? kEditorAmber
                            : Colors.white70,
                        fontSize: 13))))
            .toList(),
