...
## macOS Future Handoff From Final Windows v4.1.12 (Updated 2026-04-29)

Windows v4 is now fully user verified at commit `160d137`. macOS should not be
edited during the iOS pass, but future macOS migration must preserve the same
product contracts in macOS-native terms:
...
- Cross-mode bookmarks, visible-text search, active-STT scroll lock, stopped
  ...
  policy must be documented in macOS MVP docs before implementation.

Before major V5 feature work, plan a behavior-preserving split of macOS
`script_editor_screen.dart` and `teleprompter_screen.dart`; both exceed the
preferred maintainability ceiling.
...
When porting post-v4 Windows behavior to macOS, keep platform isolation intact
and read the matching `_agent/mvp/Platform_macOS/*.md` file before editing.
