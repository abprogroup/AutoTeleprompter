...
class WordAligner {
  // ── Tuning constants ───────────────────────────────────────────────────────
  // Keep window small to avoid false distant matches
  static const int _searchWindowSize = 8;
  // Max words to advance in a single update — prevents sentence-skipping
  static const int _maxJumpWords = 3;
  ...
  static const double _distancePenaltyPerWord = 0.06;
  ...
  static AlignmentResult align({
    ...
    // Find the search start: skip over newlines after current position
    int searchStart = lastConfirmedIndex + 1;
    while (searchStart < script.length && script[searchStart].isNewline) {
      searchStart++;
    }
