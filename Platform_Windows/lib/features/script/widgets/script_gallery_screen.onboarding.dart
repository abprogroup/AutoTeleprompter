...
const String _windowsOnboardingVersion = 'windows-onboarding-v1';
...
const List<_WindowsOnboardingStep> _windowsOnboardingSteps = [
  _WindowsOnboardingStep(
...
extension _ScriptGalleryOnboardingParts on _ScriptGalleryScreenState {
  void _scheduleWindowsOnboardingIfNeeded(AppSettings settings) {
    if (_windowsOnboardingVisible ||
        _windowsOnboardingScheduled ||
        settings.windowsOnboardingVersionSeen == _windowsOnboardingVersion) {
      return;
    }
    _windowsOnboardingScheduled = true;
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!mounted) return;
      _setGalleryState(() {
        _windowsOnboardingVisible = true;
        _windowsOnboardingStep = 0;
        _windowsOnboardingScheduled = false;
      });
    });
  }

  Future<void> _finishWindowsOnboarding() async {
    await ref
        .read(settingsProvider.notifier)
        .setWindowsOnboardingVersionSeen(_windowsOnboardingVersion);
    if (!mounted) return;
    _setGalleryState(() => _windowsOnboardingVisible = false);
  }
  ...
  Future<void> _openWalkthroughSampleScriptAfterLobbyHandoff() async {
    ...
    _setGalleryState(() {
      _windowsOnboardingVisible = false;
      _windowsOnboardingScheduled = false;
    });
  ...
  Widget _buildWindowsOnboardingOverlay(
      AppSettings settings, bool hasProAccess) {
    final step = _windowsOnboardingSteps[_windowsOnboardingStep];
    final isFirst = _windowsOnboardingStep == 0;
    final isLast = _windowsOnboardingStep == _windowsOnboardingSteps.length - 1;
    return StableWalkthroughOverlay(
      stepId: step.id,
      target: _windowsOnboardingTarget(step),
      onClose: () => unawaited(_finishWindowsOnboarding()),
      cardBuilder: (context, targetRect, constraints) {
        final cardWidth =
            constraints.maxWidth < 760 ? constraints.maxWidth - 32 : 680.0;
        final left = _windowsOnboardingCardLeft(
          targetRect: targetRect,
          cardWidth: cardWidth,
          maxWidth: constraints.maxWidth,
        );
        final top = _windowsOnboardingCardTop(
          targetRect: targetRect,
  ...
  StableWalkthroughTarget? _windowsOnboardingTarget(
      _WindowsOnboardingStep step) {
    switch (step.id) {
  ...
  double _windowsOnboardingCardLeft({
  ...
  double _windowsOnboardingCardTop({
    required Rect? targetRect,
  ...
  Widget _onboardingHeader(_WindowsOnboardingStep step, bool hasProAccess) {
    return Padding(
      ...
      child: Row(
        ...
        children: [
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  '${step.id}  •  ${_windowsOnboardingStep + 1}/${_windowsOnboardingSteps.length}'
          ...
          IconButton(
            ...
            onPressed: () => unawaited(_finishWindowsOnboarding()),
          ),
        ],
      ),
    );
        ...
        children: [
          Expanded(
            child: LinearProgressIndicator(
              value:
                  (_windowsOnboardingStep + 1) / _windowsOnboardingSteps.length,
          ...
          OutlinedButton(
            onPressed: isFirst
                ? null
                : () => _setGalleryState(() => _windowsOnboardingStep--),
