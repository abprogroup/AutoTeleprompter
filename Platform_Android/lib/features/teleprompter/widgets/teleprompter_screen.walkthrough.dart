...
const String _presenterAndroidOnboardingVersion =
    'android-presenter-onboarding-v2';
...
extension _TeleprompterWalkthroughParts on _TeleprompterScreenState {
  // Decides (exactly once) whether to show the first-run presenter
  // walkthrough. Called reactively from build() rather than a one-shot
  // initState check, because settingsProvider starts out with defaults
  // (including an empty "seen version") while its async SharedPreferences
  // load is still in flight — deciding too early on a cold start would show
  // the walkthrough again even though it was already seen and persisted.
  void _maybeShowPresenterWalkthrough(
    AppSettings settings,
    TeleprompterState tState,
  ) {
    if (widget.showWalkthroughGuide) return;
    if (_presenterWalkthroughEligibilityChecked) return;
    if (!settings.settingsLoaded) return;
    _presenterWalkthroughEligibilityChecked = true;
    // Never compete with the resume-from-last-position dialog.
    if (tState.confirmedWordIndex > 0) return;
    if (settings.androidPresenterOnboardingVersionSeen ==
        _presenterAndroidOnboardingVersion) {
      return;
    }
  ...
  Widget _buildPresenterWalkthroughOverlay() {
    ...
    return StableWalkthroughOverlay(
      stepId: 'present.android.$_presenterWalkthroughStep',
  ...
  // Remote pairing/control lives entirely outside Present mode (it's set up
  // from the gallery), so there is no on-screen button here to spotlight for
  // the final "remote is gated" step — it stays purely informational.
          ...
          Row(
            children: [
              ...
              OutlinedButton(
                onPressed: isFirst
                    ? null
                    : () => setState(() => _presenterWalkthroughStep--),
  ...
  void _closePresenterWalkthrough() {
    unawaited(
      ref
          ...
          .setAndroidPresenterOnboardingVersionSeen(
              _presenterAndroidOnboardingVersion),
    );
    setState(() => _presenterWalkthroughVisible = false);
