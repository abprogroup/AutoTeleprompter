...
class DeletedScriptsService {
  ...
  Future<List<DeletedScriptEntry>> listLocalDeletedScripts() async {
    ...
    await _migrateLegacyDeletedSourceFolder(folder);
  ...
  Future<void> permanentlyDeleteLocal(DeletedScriptEntry entry) async {
    ...
    await _deleteLegacyDeletedSourceCopies(entry.name);
    notifyChanged();
  }
  ...
  Future<void> _deleteLegacyDeletedSourceCopies(String deletedName) async {
    try {
      final backup = await _store.loadLocalBackupConnection();
      final rootPath = backup.folderPath.trim();
      if (rootPath.isEmpty) return;
      final legacyFolder = Directory(_join(rootPath, 'Deleted Source Files'));
      if (!await legacyFolder.exists()) return;
      final originalName = _originalNameFromDeletedName(deletedName);
      await for (final entity in legacyFolder.list()) {
        if (entity is! File) continue;
        final name = _basename(entity.path);
        if (name == deletedName || name == originalName) {
          await entity.delete();
        }
      }
    } catch (error, stack) {
      LightweightDiagnostics.instance.recordError(
        error,
        stack,
        source: 'deletedScripts.deleteLegacySourceCopy',
      );
    }
  }

  Future<void> _migrateLegacyDeletedSourceFolder(
      Directory deletedFolder) async {
    try {
      final backup = await _store.loadLocalBackupConnection();
      final rootPath = backup.folderPath.trim();
      if (rootPath.isEmpty) return;
      ...
      final existingOriginalNames = <String>{};
      await for (final entity in deletedFolder.list()) {
        if (entity is! File) continue;
        existingOriginalNames.add(_originalNameFromDeletedName(
      ...
      await for (final entity in legacyFolder.list()) {
        if (entity is! File) continue;
        final name = _basename(entity.path);
        if (name.toLowerCase().endsWith('.history.json')) {
          await entity.delete();
          continue;
        }
        final key = _originalNameFromDeletedName(name).toLowerCase();
        if (existingOriginalNames.contains(key)) {
          await entity.delete();
          continue;
        }
        final stamp = _stampFromDateTime((await entity.stat()).modified);
        final target =
            await _uniqueDeletedFile(deletedFolder, '${stamp}_$name');
        await _moveFile(entity, target);
        existingOriginalNames.add(key);
      }

      if (await _isDirectoryEmpty(legacyFolder)) {
      ...
      LightweightDiagnostics.instance.recordError(
        error,
        stack,
        source: 'deletedScripts.migrateLegacySourceFolder',
      );
    }
  }

  Future<File> _uniqueDeletedFile(Directory folder, String fileName) async {
    var candidate = File(_join(folder.path, fileName));
    if (!await candidate.exists()) return candidate;
    final dot = fileName.lastIndexOf('.');
    final base = dot <= 0 ? fileName : fileName.substring(0, dot);
    final ext = dot <= 0 ? '' : fileName.substring(dot);
    for (var i = 2; i < 1000; i++) {
      candidate = File(_join(folder.path, '$base ($i)$ext'));
      if (!await candidate.exists()) return candidate;
    }
    return File(_join(
      folder.path,
  ...
  Future<bool> _isDirectoryEmpty(Directory folder) async {
    await for (final _ in folder.list()) {
      return false;
    }
    return true;
  }
  ...
  static String _stampFromDateTime(DateTime value) {
