...
const whisperModels = [
  ...
  WhisperModelInfo(
      ...
      label: 'Whisper Small',
      size: '~466MB',
      description: 'More accurate. Needs a decent device.',
      model: WhisperModel.small),
  WhisperModelInfo(
      engineKey: 'whisper_medium',
      label: 'Whisper Medium',
      size: '~1.5GB',
      description: 'Most accurate. Needs a powerful device.',
      model: WhisperModel.medium),
];

/// Windows stub: Whisper offline speech-to-text is not available on Windows.
/// All methods are no-ops; the provider falls back to speech_to_text.
class WhisperSpeechService {
  ...
  Future<void> start({String? localeId, WhisperModel? model}) async {
    onError?.call('Whisper offline speech-to-text is not available on Windows');
  }
