  ...
  Future<void> _pickFile() async {
    ...
    try {
      result = await FilePicker.platform.pickFiles(
        ...
        allowedExtensions: ['txt', 'rtf', 'doc'],
        dialogTitle: 'Choose a script file',
      );
    } catch (_) {
      // Android 13+ may not recognize .rtf MIME type — fall back to any file
      try {
        result = await FilePicker.platform.pickFiles(
          type: FileType.any,
          dialogTitle: 'Choose a script file',
  ...
  Future<void> _saveScript() async {
    ...
    final name = await showDialog<String>(
      ...
      builder: (_) => AlertDialog(
        backgroundColor: const Color(0xFF1A1A1A),
        title:
            const Text('File Name', style: TextStyle(color: Colors.white)),
        content: TextField(
          controller: nameCtrl,
          autofocus: true,
    ...
    // Step 4: Save via file picker (bytes param — no crash)
    try {
      ...
      if (savedPath != null) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text('Saved: $name.$format'),
